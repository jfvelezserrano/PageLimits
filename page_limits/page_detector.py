...
class AdaptativePreprocesor(Preprocesor):
    def __binarize_image(self, image,verbose = False):
...
class SiftPointFilter(PointFilter):
    ...
    def __compute_refence_descriptors(self, samples):
        """
            ...
            Carga la imagen
            La preprocesa: binarización y apertura
        ...
        for image_gt in samples:
            ...
            corner_descriptors = sift.compute(preprocess_image,corner_keypoints)
                        
            if self.verbose:
                print(corner_descriptors[1].shape)
                plt.show()
                            
        return corner_descriptors[1]    
    ...
    def filter(self,points, image):
        image = cv2.resize(image,(image.shape[1]//self.RESIZE,image.shape[0]//self.RESIZE))        
        harris_keypoints = [cv2.KeyPoint(float(x[0]), float(x[1]), self.KEYPOINT_SIZE) for x in points]
        
        sift = cv2.xfeatures2d.SIFT_create()
        
        query_descriptors = sift.compute(image,harris_keypoints)

        for cont in range(0,query_descriptors[1].shape[0]):
        
            bf = cv2.BFMatcher(cv2.NORM_L2)
                
            matches = bf.knnMatch(query_descriptors[1], self.reference_descriptors, k=1) 
            ...
            plt.subplot(1,5,5)
            plt.imshow(image,cmap="gray")
            plt.scatter(output[:, 1], output[:, 2], c='red', marker="+")
            plt.show()
...
class PageDetector:
    ...
    def __init__(self):
        self.preprocesor = AdaptativePreprocesor()
        self.pointDetector = HarrisPointDetector()
        self.pointFilter = SiftPointFilter(self.preprocesor)
